...
def add_user(name, username, password, queQuan, **kwargs):
    ...
    email = encoding_no1(email)

    user1 = User(name=name, maSo=str(maso), username=email,
                 password=password, email=email, joined_date=datetime.now(),
                 diachi="TPHCM", userRole=UserRole.SINHVIEN, idPerson=sv.id,
                 dob=datetime.strptime(dob, '%Y-%m-%d').date(),
                 avatar=avatar, sdt=phone, sex=sex, queQuan=queQuan)
