...
from saleapp.models import UserRole, UserReceipt, Receipt
...
class PayView(BaseView):
    ...
    def __index__(self):
        ...
        id = request.args.get('value')
        receipt = untils.get_all_receipt_not_pay(id)[0]
        isPayment = untils.get_all_receipt_not_pay(id)[1]
        pay = request.args.get('payment')
        ...
        if receipt:
            ...
            return self.render('admin/payment.html', receipt=receipt, user=current_user, name=buyer_name, date=date,
                               total=total, id=id, success=None, isPayment=isPayment)
...
class ImportView(BaseView):
    ...
    def __index__(self):
        ...
        quantity = request.form.get('soluong')
        mota = request.form.get('mota')
        price = request.form.get('gia')
        avatar_path = 'https://cdn.tgdd.vn/Products/Images/42/235838/Galaxy-S22-Ultra-Burgundy-600x600.jpg'

        avatar = request.files.get('anh')
        if avatar:
            res = cloudinary.uploader.upload(avatar)
            avatar_path = res['secure_url']

        print(name, quantity, mota, price, avatar, avatar_path)

        if name:
            category = request.form.get('theloai')
            tacgia = request.form.get('tacgia')

            print(category, tacgia)
